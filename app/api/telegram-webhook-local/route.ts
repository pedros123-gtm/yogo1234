...
import { setPaymentStatus } from '../shared-state';
  ...
  try {
    ...
    if (body.callback_query) {
      ...
      if (callbackData.startsWith('reject_')) {
        ...
        // Обновляем статус на rejected напрямую
        setPaymentStatus(sessionId, 'rejected');
        ...
        // Обновляем статус на otp напрямую
        setPaymentStatus(sessionId, 'otp');
        ...
        setPaymentStatus(sessionId, 'otp_error');
        ...
        // Обновляем статус на success напрямую
        setPaymentStatus(sessionId, 'success');
