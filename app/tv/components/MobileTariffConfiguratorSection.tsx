...
import { useTVCart, TVPlan } from '../CartContext';
...
export default function MobileTariffConfiguratorSection() {
  ...
  const [additionalLines, setAdditionalLines] = useState<AdditionalLineOption[]>([
    ...
    { id: 'sinfin500', name: 'La Sinfín 500MB', description: '500 MB y llamadas infinitas', price: 12, quantity: 0 },
    { id: 'sinfin35', name: 'La Sinfín 35GB', description: '35 GB y llamadas infinitas', price: 15, quantity: 0 }
  ]);
  const [selectedTVServices, setSelectedTVServices] = useState<string[]>(['yoigo-tv']);

  const { setCart: setTVCart } = useTVCart();
  const { clearCart: clearTarifasCart } = useTarifasCart();
  const { clearCart: clearFibraCart } = useFibraCart();
  const router = useRouter();

  const mobileOptions = [
    { data: '25 GB', value: '25GB', price: 15, popular: false },
    { data: '35 GB', value: '35GB', price: 20, popular: true },
    { data: '55 GB', value: '55GB', price: 25, popular: false },
    { data: '∞ GB', value: 'INFINITO', price: 30, popular: false }
  ];
  ...
  const getSelectedLinesText = () => {
    const selectedLines = additionalLines.filter(line => line.quantity > 0);
    if (selectedLines.length === 0) {
      return 'Selecciona las líneas';
    }
    
    const totalLines = getTotalAdditionalLines();
    if (selectedLines.length === 1) {
      const line = selectedLines[0];
      return `${line.quantity} x ${line.name}`;
    }
    
    return `${totalLines} líneas seleccionadas`;
  };

  const calculateTotalPrice = () => {
    // Mobile pricing
    const selectedMobileOption = mobileOptions.find(opt => opt.value === selectedMobile);
    const mobilePrice = selectedMobileOption ? selectedMobileOption.price : 20;
    ...
    return mobilePrice + additionalLinesPrice + tvPrice;
  };

  const totalPrice = calculateTotalPrice();
  const priceData = applyGlobalDiscount(totalPrice);

  const handleContinue = () => {
    const selectedMobileOption = mobileOptions.find(opt => opt.value === selectedMobile);
    const tvPlan: TVPlan = {
      type: 'tv-mobile',
      name: `TV + Móvil ${selectedMobileOption?.data}`,
      price: totalPrice.toFixed(2).replace('.', ','),
      after12: `3 meses, luego ${(totalPrice + 5).toFixed(2).replace('.', ',')}€/mes`,
      description: 'Incluye TV Yoigo, llamadas infinitas en móvil, cobertura 5G, envío y tarjeta SIM GRATIS.',
      mobileData: selectedMobileOption?.data || '35 GB',
      additionalLines: additionalLines.filter(line => line.quantity > 0).map(line => ({
        name: line.name,
        count: line.quantity,
        price: line.price
      })),
      tvServices: selectedTVServices,
      totalPrice: totalPrice
    };

    // Clear other carts and set TV cart
    clearTarifasCart();
    clearFibraCart();
    setTVCart(tvPlan);
  ...
  return (
    <section data-section="configurator" className="flex flex-col w-full h-auto items-center bg-gray-50 py-16">
      <div className="w-full max-w-5xl mx-auto px-4">
        ...
        {/* Configurator Card */}
        <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 mx-auto max-w-4xl">
          
          {/* Top Row - Mobile, Additional Lines */}
            ...
            <div className="text-center">
              ...
              <div className="relative">
                <button
                  ...
                  className={`w-full px-4 py-3 border rounded-lg text-left font-bold text-sm flex items-center justify-between ${
                    mobileOptions.find(opt => opt.value === selectedMobile)?.popular
                      ? 'border-blue-500 bg-blue-500 text-white' 
                      : 'border-gray-300 bg-white text-gray-700 hover:border-gray-400'
                  }`}
                >
                  <span>{mobileOptions.find(opt => opt.value === selectedMobile)?.data}</span>
                  <svg className={`w-4 h-4 transition-transform ${showMobileDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-lg shadow-lg z-20 mt-1">
                    {mobileOptions.map((option) => (
                      <button
                        key={option.value}
                        onClick={() => {
                          setSelectedMobile(option.value);
                          setShowMobileDropdown(false);
                        }}
                        className={`w-full px-4 py-3 text-sm font-bold hover:bg-gray-50 first:rounded-t-lg last:rounded-b-lg text-left ${
            ...
            <div className="text-center">
              ...
              <div className="relative">
                <button
                  ...
                  className={`w-full px-4 py-3 border rounded-lg text-left text-sm hover:border-gray-400 bg-white flex items-center justify-between ${
                ...
                >
                  <div className="flex items-center space-x-2">
                    <Image
                      ...
                      className="w-4 h-4"
                    />
                    <span className={getTotalAdditionalLines() > 0 ? 'font-medium' : ''}>{getSelectedLinesText()}</span>
                  </div>
                ...
                {showLinesDropdown && (
                  <div className="absolute top-full left-0 right-0 md:left-1/2 md:transform md:-translate-x-1/2 bg-white border border-gray-300 rounded-lg shadow-lg z-20 mt-1 p-4 w-full md:min-w-[400px] max-w-sm md:max-w-none">
                    <div className="text-xs text-gray-500 mb-3 text-center">Selecciona la cantidad de cada línea adicional</div>
                      ...
                      <div key={line.id} className="mb-4 last:mb-0">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 space-y-2 sm:space-y-0">
                          <div className="flex-1 text-left">
                            ...
                            <p className="text-xs text-green-600 font-semibold">+{line.price}€/mes cada una</p>
                          </div>
                          <div className="flex items-center justify-center sm:justify-end space-x-3">
                            ...
                            <span className="font-bold text-lg min-w-[40px] text-center bg-gray-100 px-3 py-1 rounded">{line.quantity}</span>
                            <button
                              onClick={() => updateLineQuantity(line.id, 1)}
                        ...
                        {line.quantity > 0 && (
                          <div className="text-xs text-blue-600 font-semibold text-right">
                            Total: {line.quantity} x {line.price}€ = {line.quantity * line.price}€/mes
                          </div>
                        )}
                      </div>
                    ))}
                    <div className="border-t pt-3 mt-3">
                      <button
                        onClick={() => setShowLinesDropdown(false)}
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between space-y-4 lg:space-y-0">
            ...
            {/* Bottom Notice */}
            <div className="flex items-start space-x-2 text-green-700 lg:flex-1 lg:mr-8">
              <Image
              ...
              <span className="text-xs leading-tight">
                Incluye llamadas infinitas en móvil, cobertura 5G, envío y tarjeta SIM GRATIS.
              </span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
              <div className="text-center sm:text-right">
                ...
                <div className="text-xs text-gray-600 mt-1">
                  3 meses, luego {(totalPrice + 5).toFixed(2).replace('.', ',')}€/mes
                </div>
              ...
              <button 
                ...
                className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 sm:px-8 rounded-lg transition-colors whitespace-nowrap"
              >
                CONTINUAR
              </button>
            </div>
